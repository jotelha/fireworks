    ...
    def check_dataflow(self):
        ...
        for vertex in list(self.vs):
            for entity in vertex['inputs']:
                ...
                # if a (Py-) task has a key in its 'inputs' that just refers
                # to a static fw_spec field (or a field set differently,
                # not via any previous 'outputs'), then we would not want
                # this method to fail, but print a warning (?).
                if len(sources) != 1:
                    warnings.warn(', '.join((
                        'An input field should have exactly one source',
                        'step "{}", entity "{}"'.format(vertex['name'], entity),
